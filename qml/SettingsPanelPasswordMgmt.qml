...
StyledExpansionPanel {
    ...
    function acceptableInput() {
        if (!!yubiKey.currentDevice && yubiKey.currentDeviceValidated) {
            if (!!yubiKey.currentDevice && yubiKey.currentDevice.hasPassword && currentPasswordField.text.length == 0)
                return false;

            if (newPasswordField.text.length > 0 && (newPasswordField.text === confirmPasswordField.text))
                return true;
