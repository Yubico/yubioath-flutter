...
ListModel {
    ...
    function listModelInsertSortedArray(listModel, compareFunc, arr) {
        for (var i = 0; i < arr.length; i++) listModelInsertSorted(listModel, compareFunc, arr[i])
    }

    function listModelInsertSortedListModel(listModel, compareFunc, model) {
        for (var i = 0; i < model.count; i++) listModelInsertSorted(listModel, compareFunc, model.get(i))
    }
