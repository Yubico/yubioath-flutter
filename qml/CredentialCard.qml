...
Pane {
    ...
    property bool hotpCredential: !!credential && (credential && credential.oath_type === "HOTP")
    property bool hotpCredentialInCoolDown
    property bool customPeriodCredentialNoTouch: !!credential && (credential.period !== 30 && credential.oath_type === "TOTP" && !touchCredential)
    ...
    function toggleFavorite() {
        if (favorite) {
            settings.favorites = settings.favorites.filter((fav) => {
                return fav !== credential.key;
            });
        } else {
    ...
    function calculateCard(copy) {
        if (touchCredentialNoCode || (hotpCredential && !hotpCredentialInCoolDown) || customPeriodCredentialNoTouch) {
            ...
            "acceptedCb": function() {
                yubiKey.deleteCredential(credential, function(resp) {
                    if (resp.success) {
                        ...
                        navigator.snackBar(qsTr("Account deleted"));
                    } else {
                        navigator.snackBarError(navigator.getErrorMessage(resp.error_id));
    ...
    background: Rectangle {
        ...
        opacity: if (credentialCard.GridView.isCurrentItem)
            ...
            return isDark() ? cardNormalEmphasis : 1;
