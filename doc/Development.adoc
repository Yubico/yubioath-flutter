...
Building for macOS requires specific Apple developer tools and package managers:

Xcode (Full Application): Required to compile native Swift/Objective-C code. The "Command Line Tools" alone are insufficient for Flutter desktop builds. Install via the Mac App Store.

CocoaPods: A dependency manager used by Flutter to link native macOS hardware libraries.

UV: A Python package installer used to build the Yubico Authenticator Helper.
...
Python >= 3.10 and UV. This needs to be built prior to running
...
==== macOS
    ...
    $ brew install swig uv cocoapods
